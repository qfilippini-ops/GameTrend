    ...
    <div className="space-y-3">
      ...
      <AnimatePresence>
          ...
          >
            {results.map((r) => (
              <div key={r.id} className="flex items-center gap-3 p-3 rounded-2xl border border-surface-700/30 bg-surface-900/50">
                ...
                {sent.has(r.id) ? (
                  <span className="text-xs text-emerald-400 font-medium px-3 py-1.5 rounded-xl bg-emerald-950/30 border border-emerald-700/30">{t("sent")}</span>
                ) : (
                  <button
        ...
        {query.trim().length >= 2 && !searching && results.length === 0 && (
          <p className="text-surface-600 text-sm text-center py-3">{t("searchNoResults")}</p>
        )}
      </AnimatePresence>
    </div>
  );
    ...
    <div className="min-h-screen bg-surface-950 bg-grid">
      ...
      <div className="px-4 pt-4 pb-8 space-y-5 max-w-lg mx-auto">
                ...
                {pendingRequests.map((notif) => (
                  <div key={notif.id} className="flex items-center gap-3 px-4 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-surface-500 text-xs">{t("wantToBeFriend")}</p>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <button
        ...
        <div className="rounded-2xl border border-surface-700/40 bg-surface-900/50 p-4">
          <p className="text-white font-display font-bold text-sm mb-3">{t("findPlayer")}</p>
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <p className="text-surface-400 text-xs font-medium uppercase tracking-widest">
              {t("listTitle")} · {t("listMax", { n: friends.length })}
            </p>
          ...
          {loading ? (
            ...
            <div className="text-center py-12 rounded-2xl border border-dashed border-surface-700/30 bg-surface-900/20">
              ...
              <p className="text-surface-500 text-sm">{t("noFriendsTitle")}</p>
              <p className="text-surface-600 text-xs mt-1">{t("noFriendsHint")}</p>
            </div>
          ) : (
            <div className="space-y-2">
              {/* En ligne / en partie */}
              {online.length > 0 && (
                <>
                  <p className="text-surface-600 text-xs px-1 mb-1">{t("online")}</p>
                  {online.map((f) => (
              ...
              {offline.length > 0 && (
                <>
                  {online.length > 0 && <p className="text-surface-600 text-xs px-1 mt-3 mb-1">{t("offline")}</p>}
                  {offline.map((f) => (
                    <FriendCard key={f.user_id} friend={f} onRemove={handleRemove} />
                  ))}
                </>
