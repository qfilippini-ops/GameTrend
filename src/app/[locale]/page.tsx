...
export default async function LandingPage({
  ...
  // ── JSON-LD : signal majeur pour Google. WebSite avec SearchAction permet
  // d'avoir une "sitelinks searchbox" dans les résultats de recherche.
  // Organization aide à la consolidation marque (Knowledge Panel à terme).
  const websiteLd = {
    ...
    potentialAction: {
      ...
      target: {
        "@type": "EntryPoint",
        urlTemplate: `${SITE_URL}/${locale}/presets?query={search_term_string}`,
      },
  ...
  return (
    <div className="bg-grid min-h-screen">
      ...
      <div className="px-4 pt-4 pb-8 space-y-5">
        ...
        {/* ── HERO ─────────────────────────────────────────────────────────── */}
        <section className="relative rounded-2xl bg-gradient-to-br from-brand-950 via-surface-900 to-ghost-950 border border-brand-700/20 px-5 py-4 overflow-hidden">
          ...
          <div className="relative z-10 space-y-3">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                ...
                {/*
                  H1 unique optimisé SEO : combine 3 segments visuels mais reste
                  un seul élément `<h1>` côté DOM (cf. balisage explicite).
                  Mots-clés cibles : "jeux de soirée", "entre amis", "créer".
                */}
                <h1 className="text-2xl font-display font-bold text-white leading-tight tracking-tight">
                  {t("heroH1Part1")}{" "}
                  <span className="text-gradient-brand">{t("heroH1Part2")}</span>{" "}
                  {t("heroH1Part3")}
                </h1>
              </div>
