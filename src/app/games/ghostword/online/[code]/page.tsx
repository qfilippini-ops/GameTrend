...
function JoinScreen({
  ...
  async function doJoin(
    ...
    if (!room) { setError("Salon introuvable"); setAutoJoining(false); setLoading(false); return; }
    if (room.phase !== "lobby") { setError("La partie a déjà commencé"); setAutoJoining(false); setLoading(false); return; }
    ...
    if (taken) { setError("Ce pseudo est déjà pris dans ce salon"); setAutoJoining(false); setLoading(false); return; }
  ...
  async function handleJoin() {
    const n = name.trim();
    if (!n) { setError("Entre ton pseudo"); return; }
    setLoading(true);
    setError("");
  ...
  if (autoJoining && !error) {
    return (
      <div className="min-h-screen bg-surface-950 flex items-center justify-center">
        <p className="text-surface-500 text-sm animate-pulse">Connexion au salon…</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-surface-950 bg-grid flex items-center justify-center px-5">
      <div className="w-full max-w-sm space-y-4">
        <div className="text-center">
          <p className="text-surface-500 text-xs uppercase tracking-widest mb-2">Salon</p>
        ...
        <div className="rounded-2xl border border-surface-700/40 bg-surface-900/60 p-4 space-y-3">
          <label className="text-white font-display font-bold text-sm">Ton pseudo</label>
          <input type="text" value={name} onChange={(e) => setName(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && handleJoin()}
            placeholder="Entrez votre pseudo" maxLength={20} autoFocus
...
export default function RoomPage() {
  ...
  useEffect(() => {
    ...
    switch (evt.type) {
      case "player_left":
        msg = `🚪 ${evt.player} a quitté la partie`;
        break;
      case "host_left":
        msg = `🚪 ${evt.player} (hôte) a quitté la partie`;
        if (evt.new_host === myName) {
          msg += " — Tu es maintenant l'hôte 👑";
        } else if (evt.new_host) {
          msg += ` — ${evt.new_host} est l'hôte`;
        }
        break;
      case "kicked":
        msg = `🚷 ${evt.player} a été expulsé`;
        break;
    }
  ...
  if (!myName) {
    return (
      <div className="min-h-screen bg-surface-950 flex items-center justify-center">
        <p className="text-surface-500 text-sm animate-pulse">Identification…</p>
      </div>
    );
  }
