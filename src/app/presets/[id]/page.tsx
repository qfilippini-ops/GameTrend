...
const ROLE_STYLE: Record<string, { bg: string; border: string; emoji: string; label: string }> = {
  initie: { bg: "bg-brand-900/40",  border: "border-brand-700/40",  emoji: "🧠", label: "Initiés" },
  ombre:  { bg: "bg-ghost-900/40",  border: "border-ghost-700/40",  emoji: "👻", label: "Ombre" },
  vide:   { bg: "bg-surface-800/60", border: "border-surface-700/40", emoji: "💨", label: "Le Vide" },
};
      ...
      <div className="px-4 space-y-6 pb-12 -mt-1">
        ...
        <div className="flex items-center justify-between pt-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-surface-400 text-sm">{author?.username ?? "Anonyme"}</span>
        ...
        <div className="flex flex-wrap gap-2 -mt-2">
          {isGhostWord && ghostConfig?.families && (
            <>
              <div className="px-3 py-1.5 rounded-full bg-surface-800/60 border border-surface-700/40 text-xs text-surface-400">
                📝 {ghostConfig.families.length} famille{ghostConfig.families.length > 1 ? "s" : ""}
              </div>
              <div className="px-3 py-1.5 rounded-full bg-surface-800/60 border border-surface-700/40 text-xs text-surface-400">
          ...
          {isDYP && dypConfig?.cards && (
            <div className="px-3 py-1.5 rounded-full bg-surface-800/60 border border-surface-700/40 text-xs text-surface-400">
              🃏 {dypConfig.cards.length} carte{dypConfig.cards.length > 1 ? "s" : ""}
            </div>
          )}
          {preset.is_public ? (
        ...
        >
          Jouer avec ce preset {gameMeta.icon}
        </Link>
        ...
        {isGhostWord && ghostConfig?.families && ghostConfig.families.length > 0 && (
          <div>
            <SectionHeader
              ...
              badge={`${ghostConfig.families.length} famille${ghostConfig.families.length > 1 ? "s" : ""}`}
            />
                ...
                <details key={family.id} className="rounded-2xl border border-surface-700/40 bg-surface-900/50 overflow-hidden group">
                  <summary className="flex items-center gap-3 px-4 py-3 cursor-pointer select-none list-none hover:bg-surface-800/30 transition-colors">
                    <span className="flex-1 text-sm font-semibold text-white">
                      {family.name || <span className="text-surface-500 italic">Sans nom</span>}
                    </span>
                    <span className="text-surface-600 text-xs font-mono">{family.words.length} mot{family.words.length > 1 ? "s" : ""}</span>
        ...
        {isGhostWord && ghostConfig?.roles && (
          <div>
            <SectionHeader emoji="🎭" title="Rôles" />
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-center">
                      <p className="text-[10px] text-surface-500 uppercase tracking-wide">{style.label}</p>
        ...
        {isDYP && dypConfig?.cards && (
          <div>
            <SectionHeader
              emoji="🃏"
              title="Cartes"
              badge={`${dypConfig.cards.length} carte${dypConfig.cards.length > 1 ? "s" : ""}`}
            />
            <details className="rounded-2xl border border-surface-700/40 bg-surface-900/50 overflow-hidden group">
              <summary className="flex items-center justify-between px-4 py-3 cursor-pointer select-none list-none hover:bg-surface-800/30 transition-colors">
                <span className="text-sm font-semibold text-white">Voir toutes les cartes</span>
                <span className="text-surface-600 text-xs transition-transform group-open:rotate-180">▼</span>
              </summary>
        ...
        {isDYP && (
          <div>
            <SectionHeader
              emoji="🏆"
              title="Palmarès"
              badge={totalDYPGames > 0 ? `${totalDYPGames} partie${totalDYPGames > 1 ? "s" : ""}` : undefined}
            />
            <details className="rounded-2xl border border-surface-700/40 bg-surface-900/50 overflow-hidden group">
              <summary className="flex items-center justify-between px-4 py-3 cursor-pointer select-none list-none hover:bg-surface-800/30 transition-colors">
                <span className="text-sm font-semibold text-white">
                  {topCards.length > 0 ? `🥇 ${topCards[0]?.name}` : "Aucune donnée"}
                </span>
                <span className="text-surface-600 text-xs transition-transform group-open:rotate-180">▼</span>
              </summary>
              <div className="border-t border-surface-800/60">
                {topCards.length === 0 ? (
                  <div className="px-5 py-8 text-center">
                    ...
                    <p className="text-surface-500 text-sm">Aucune partie jouée encore.</p>
                    <p className="text-surface-600 text-xs mt-1">Lance une partie pour voir le classement !</p>
                  </div>
                ) : (
                  <>
                    {/* Podium top 3 */}
        ...
        {isOwner && (
          <div className="space-y-2.5 pt-2 border-t border-surface-800/60">
            <p className="text-xs text-surface-600 uppercase tracking-wide">Gestion</p>
            <Link
              href={`/presets/${preset.id}/edit`}
